...
use crate::ast::{self, standard::{BlockStatement, IfExpression, Node, Program}};
...
fn eval(node: Node, env: &mut Environment) -> Object {
    match node {
        ...
        Node::Statement(statement) => match statement {
            ast::standard::Statement::LetStatement(statement) => {
                if statement.expression.is_none() {
                    return Object::Error("expected expression".into());
                }

                let value = eval(ast::standard::Node::Expression(statement.expression.unwrap()), env);
            ...
            ast::standard::Statement::ReturnStatement(ret) => {
                Object::Return(Box::new(eval(ast::standard::Node::Expression(ret.expression), env)))
            }
            ast::standard::Statement::ExpressionStatement(expression) => {
                eval(ast::standard::Node::Expression(expression), env)
            }
            ast::standard::Statement::BlockStatement(block) => eval_block_statement(block, env),
        },
        Node::Expression(expression) => match expression {
            ast::standard::Expression::Identifier(identifier) => eval_identifier(identifier, env),
            ast::standard::Expression::StringLiteral(_) => todo!(),
            ast::standard::Expression::IntegerLiteral(i) => Object::Integer(i),
            ast::standard::Expression::Prefix(prefix) => {
                let right = eval(ast::standard::Node::Expression(*prefix.right), env);
            ...
            ast::standard::Expression::Infix(infix) => {
                let left = eval(ast::standard::Node::Expression(*infix.left), env);
                let right = eval(ast::standard::Node::Expression(*infix.right), env);
            ...
            ast::standard::Expression::Empty => todo!(),
            ast::standard::Expression::Boolean(boolean) => Object::Boolean(boolean),
            ast::standard::Expression::IfExpression(ifexp) => eval_if_expression(ifexp, env),
            ast::standard::Expression::FunctionLiteral(function) => Object::Function(object::Function {
                parameters: function.parameters,
                body: function.body,
                env: env.clone(),
            }),
            ast::standard::Expression::CallExpression(function) => {
                let fnn = eval(ast::standard::Node::Expression(*function.function), env);
...
fn eval_statements(statements: Vec<ast::standard::Statement>, env: &mut Environment) -> Object {
    let mut result = Object::Null;

    for statement in statements {
        result = eval(ast::standard::Node::Statement(statement), env);
...
fn eval_if_expression(exp: IfExpression, env: &mut Environment) -> Object {
    let condition = eval(ast::standard::Node::Expression(*exp.condition), env);

    if is_truthy(condition) {
        eval(
            ast::standard::Node::Statement(ast::standard::Statement::BlockStatement(exp.consequence)),
            env,
        )
    } else if exp.alternative.is_some() {
        return eval(
            ast::standard::Node::Statement(ast::standard::Statement::BlockStatement(exp.alternative.unwrap())),
            env,
        );
    } else {
        return Object::Null;
...
fn eval_identifier(identifier: ast::standard::Identifier, env: &mut Environment) -> Object {
...
fn eval_expressions(expressions: Vec<ast::standard::Expression>, env: &mut Environment) -> Vec<Object> {
    let mut result = Vec::new();

    for expression in expressions {
        let evaluated = eval(ast::standard::Node::Expression(expression), env);
...
fn apply_function(function: Object, args: Vec<Object>) -> Object {
    match function {
        Object::Function(f) => {
            ...
            let evaluated = eval(
                ast::standard::Node::Statement(ast::standard::Statement::BlockStatement(f.body)),
                &mut extended_env,
            );
...
mod tests {
    ...
    fn test_integer_expression() -> Result<()> {
        ...
        for (input, expected) in tests {
            ...
            let evaluation = eval(ast::standard::Node::Program(program), &mut environment);
